...
async function loadLibrary() {
  try {
    const snap = await fb.getDocs(fb.collection(_db, 'hub_assets'));
    window._libAssets = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    renderLibrary('all');
  } catch(e) {
    const grid = document.getElementById('libGrid');
    if (grid) grid.innerHTML = '<p class="empty-msg" style="grid-column:1/-1">Could not load library.</p>';
  }
}
...
// ── SIDEBAR FEATURE DROPDOWNS ─────────────────────────────
document.querySelectorAll('.sb-feat-hd').forEach(btn => {
  btn.addEventListener('click', () => {
    const grp = document.getElementById(btn.dataset.grp);
    const isOpen = grp.classList.contains('open');
    // Close all
    document.querySelectorAll('.sb-feat-group.open').forEach(g => g.classList.remove('open'));
    if (!isOpen) grp.classList.add('open');
  });
});

document.querySelectorAll('.sb-feat-item[data-page]').forEach(btn => {
  btn.addEventListener('click', () => {
    const page = btn.dataset.page;
    showPage(page);
    document.getElementById('mobTitle').textContent = btn.textContent.trim();
    document.getElementById('sidebar').classList.remove('mob-open');
    document.getElementById('sidebarOverlay').hidden = true;
    // Handle sub-tab shortcuts
    if (page === 'library' && btn.dataset.libtab) switchLibTab(btn.dataset.libtab);
    if (page === 'translate' && btn.dataset.trans) {
      document.getElementById('transDest').value = btn.dataset.trans;
    }
  });
});
...
function ytPlay(input) {
  const parsed = getYouTubeId(input);
  if (!parsed) { toast('Paste a valid YouTube URL or video ID', 'warn'); return; }
  ...
  const src = parsed.type === 'list'
    ? `https://www.youtube-nocookie.com/embed/videoseries?list=${parsed.id}&autoplay=1&rel=0&modestbranding=1`
    : `https://www.youtube-nocookie.com/embed/${parsed.id}?autoplay=1&rel=0&modestbranding=1&enablejsapi=1&playsinline=1`;
  ytFrame.src = src;
...
document.getElementById('ytPlayBtn').addEventListener('click', () => ytPlay(document.getElementById('ytInput').value));
document.getElementById('ytInput').addEventListener('keydown', e => { if (e.key === 'Enter') ytPlay(document.getElementById('ytInput').value); });

document.querySelectorAll('.yt-preset').forEach(btn => {
  btn.addEventListener('click', () => {
    const vid = btn.dataset.vid || '';
    const list = btn.dataset.list || '';
    ytPlay(list ? `https://youtube.com/playlist?list=${list}` : vid);
  });
});
...
document.getElementById('ytClose').addEventListener('click', () => {
  ...
  ytFrame.src = '';
  ytFrame.hidden = true;
  document.getElementById('ytEmpty').style.display = '';
  document.getElementById('ytBar').hidden = true;
});

document.getElementById('ytPiP').addEventListener('click', async () => {
  const ytFrame = document.getElementById('ytFrame');
  if (ytFrame.hidden) { toast('Start a video first', 'warn'); return; }
  try {
    const video = ytFrame.contentDocument?.querySelector('video');
    if (video && document.pictureInPictureEnabled) {
      await video.requestPictureInPicture();
    } else {
      toast('PiP not available for this video', 'warn');
    }
  } catch { toast('PiP not supported here', 'warn'); }
});
...
if (transInput) {
  transInput.addEventListener('input', () => {
    document.getElementById('transCharCount').textContent = `${transInput.value.length} / 500`;
  });
}

async function doHubTranslate() {
  const text = document.getElementById('transInput')?.value.trim();
  if (!text) return;
  const src  = document.getElementById('transSrc').value;
  ...
  out.innerHTML = '<span style="color:var(--text-mute)">Translating…</span>';
  try {
    const res  = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=${src}|${dest}`);
    ...
    document.getElementById('transCopyWrap').hidden = false;
  } catch(e) {
...
document.getElementById('transBtn')?.addEventListener('click', doHubTranslate);
document.getElementById('transInput')?.addEventListener('keydown', e => { if (e.key === 'Enter' && e.ctrlKey) doHubTranslate(); });

document.getElementById('transSwapBtn')?.addEventListener('click', () => {
  const src  = document.getElementById('transSrc');
  const dest = document.getElementById('transDest');
  const tmp  = src.value; src.value = dest.value; dest.value = tmp;
  const outText = document.getElementById('transOutput').textContent.trim();
  if (outText && outText !== 'Translation appears here…') {
    document.getElementById('transInput').value = outText;
    document.getElementById('transCharCount').textContent = `${outText.length} / 500`;
    document.getElementById('transOutput').innerHTML = '<p class="trans-placeholder">Translation appears here…</p>';
    document.getElementById('transCopyWrap').hidden = true;
  }
});

document.getElementById('transCopyBtn')?.addEventListener('click', () => {
  const text = document.getElementById('transOutput').textContent.trim();
...
document.querySelectorAll('.phrase-item').forEach(item => {
  item.addEventListener('click', () => {
    const ti = item.dataset.ti;
    const en = item.dataset.en;
    document.getElementById('transSrc').value  = 'en';
    document.getElementById('transDest').value = 'ti';
    document.getElementById('transInput').value = en;
    document.getElementById('transCharCount').textContent = `${en.length} / 500`;
    document.getElementById('transOutput').textContent = ti;
    document.getElementById('transCopyWrap').hidden = false;
  });
});
...
document.querySelectorAll('.radio-play-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('radioNowName').textContent = name;
    document.getElementById('radioNowIco').textContent  = ico;
    bar.hidden  = false;
    frame.hidden = true;

    if (type === 'audio' && stream) {
      audio.src = stream;
      audio.play().catch(() => toast('Stream not available', 'warn'));
    } else if (type === 'yt' && card.dataset.vid) {
      audio.pause(); audio.src = '';
      frame.src = `https://www.youtube-nocookie.com/embed/${card.dataset.vid}?autoplay=1&rel=0&modestbranding=1`;
      frame.hidden = false;
    } else {
      toast('Stream URL not available for this station', 'warn');
    }
  });
});
...
document.getElementById('radioClose')?.addEventListener('click', () => {
  ...
  document.getElementById('radioPlayerBar').hidden = true;
});
...
function renderLibrary(filter = 'all') {
  const grid = document.getElementById('libGrid');
  if (!grid) return;
  const assets = window._libAssets || [];
  const filtered = filter === 'all' ? assets : assets.filter(a => {
    const t = a.type || '';
    if (filter === 'music')  return t.startsWith('audio/');
    if (filter === 'videos') return t.startsWith('video/');
    if (filter === 'docs')   return !t.startsWith('audio/') && !t.startsWith('video/') && !t.startsWith('image/');
    return true;
  });
  if (!filtered.length) {
    grid.innerHTML = '<p class="empty-msg" style="grid-column:1/-1">No files here. <button class="link-btn" onclick="showPage(\'assets\')">Upload in Assets →</button></p>';
    return;
  }
  grid.innerHTML = filtered.map(a => `
    <div class="asset-card">
      <div class="asset-thumb">${a.type?.startsWith('image/') ? `<img src="${a.url}" loading="lazy"/>` : `<span style="font-size:2.5rem">${fileIcon(a.type)}</span>`}</div>
      <div class="asset-info"><div class="asset-name">${esc(a.name)}</div><div class="asset-size">${fmtBytes(a.size||0)}</div></div>
      <div class="asset-actions">
        <button class="asset-copy" onclick="navigator.clipboard.writeText('${a.url}').then(()=>toast('Copied!','success'))">Copy</button>
        <a href="${a.url}" target="_blank" class="btn-sm asset-copy" style="display:flex;align-items:center;justify-content:center">Open</a>
      </div>
    </div>`).join('');
}

document.querySelectorAll('.lib-tab').forEach(tab => {
  tab.addEventListener('click', () => switchLibTab(tab.dataset.libtab));
});

// ── SETTINGS: THEME, COLOR, LANGUAGE ─────────────────────
function applyAccentColor(hex) {
  const r = parseInt(hex.slice(1,3),16), g = parseInt(hex.slice(3,5),16), b = parseInt(hex.slice(5,7),16);
  document.documentElement.style.setProperty('--accent', hex);
  document.documentElement.style.setProperty('--accent2', shadeHex(hex, -25));
  document.documentElement.style.setProperty('--accent-glow', `rgba(${r},${g},${b},0.3)`);
  localStorage.setItem('hub_accent', hex);
}
...
document.querySelectorAll('.color-swatch').forEach(sw => {
  sw.addEventListener('click', () => {
    ...
    document.getElementById('setAccentColor').value = sw.dataset.color;
  });
});
...
document.getElementById('saveLangBtn')?.addEventListener('click', () => {
  ...
  // Apply via Google Translate widget if loaded
  const sel = document.querySelector('.goog-te-combo');
  if (sel && lang !== 'en') { sel.value = lang; sel.dispatchEvent(new Event('change')); }
  toast('Language applied!', 'success');
});
